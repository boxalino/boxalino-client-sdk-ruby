...
class BxChooseResponse
  ...
  def getSearchResultHitIds(searchResult, fieldId='id')
    ids = Array.new
    if(!searchResult.nil? || (searchResult && !searchResult.empty?))
      if(!searchResult.hits.nil? || (searchResult.hits && !searchResult.hits.empty?))
        searchResult.hits.each do |item|
  ...
  def areThereSubPhrases(choice=nil, count=0, maxBaseResults=0)
    variant = getChoiceResponseVariant(choice, count)
    results = variant.searchRelaxation
    return results && results.subphrasesResults && results.subphrasesResults != nil && results.subphrasesResults.size > 0 && getTotalHitCount(choice, false, count) <= maxBaseResults
